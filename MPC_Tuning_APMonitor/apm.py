...
if ver == 2:  # Python 2
    ...
    def apm_t0(server, app, mode):
        ...
        app.replace(" ", "")
        url = string.strip(
            server) + '/online/' + ip + '_' + app + '/' + string.strip(
                mode) + '.t0'
    ...
    def get_solution(server, app):
        ...
        app.replace(" ", "")
        url = string.strip(
            server) + '/online/' + ip + '_' + app + '/results.csv'
        f = urllib.urlopen(url)
        # Send request to web-server
        solution = f.read()
    ...
    def get_file(server, app, filename):
        ...
        app.replace(" ", "")
        url = string.strip(
            server) + '/online/' + ip + '_' + app + '/' + filename
        f = urllib.urlopen(url)
        # Send request to web-server
        file = f.read()
        # Write the file
    ...
    def web(server, app):
        ...
        url = string.strip(
            server
        ) + '/online/' + ip + '_' + app + '/' + ip + '_' + app + '_oper.htm'
    ...
    def web_var(server, app):
        ...
        url = string.strip(
            server
        ) + '/online/' + ip + '_' + app + '/' + ip + '_' + app + '_var.htm'
        webbrowser.get().open_new_tab(url)
        return url
    ...
    def apm_t0(server, app, mode):
        ...
        app = app.lower()
        app.replace(" ", "")
        url = server.strip() + '/online/' + ip + '_' + app + '/' + mode.strip(
        ) + '.t0'
...
def plotter(y, subplots=1, save=False, filename='solution', format='png'):
    ...
    try:
        ...
        for i in range(var_size):
            if list(y)[i] != 'time' and list(y)[i][:3] != 'slk':
                if j == 1:
                    ...
                    if save:
                        if pltcount != 0:
                            plt.savefig(filename + str(pltcount) + '.' +
                                        format,
                                        format=format)
                        pltcount += 1
                    plt.figure()
        ...
        plt.xlabel('time')
        if save:
            plt.savefig('plots/' + filename + str(pltcount) + '.' + format,
                        format=format)
        if pltcount <= 20:
            plt.show()
    except ImportError:
